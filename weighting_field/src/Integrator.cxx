...
namespace Integrator {
  ...
  T::data_t convolve(T::data_t signal_t, 
    ...
    std::vector<T::data_t> integrand_interpolated;
    for(int ind = 0; ind < trajectory_interpolated.size(); ind++) {
      ...
      // compute integrand
      T::data_t cur_integrand = -cur_charge / Qw * std::inner_product(cur_wf.begin(), cur_wf.end(), cur_velocity.begin(), 0.0);
      integrand_interpolated.push_back(cur_integrand);
    }

    // perform the integration
    T::data_t signal = 0;
    for(int ind = 0; ind < integrand_interpolated.size() - 1; ind++) {
      T::data_t val_l = integrand_interpolated[ind];
      T::data_t val_r = integrand_interpolated[ind + 1];
      T::data_t time_l = trajectory_interpolated[ind][0];
      T::data_t time_r = trajectory_interpolated[ind + 1][0];
